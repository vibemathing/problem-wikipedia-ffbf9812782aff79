...
继承 `governance/standards/VERIFIED_AI_MATHEMATICAL_RESEARCH_FOUNDATION.md`：按 D01–D11 定位当前阶段，一次只路由一个最短可验证义务；Router 不得把 Job、会话、PR 或 verifier 成功自动解释为 Result 准入。
<!-- FORMAL_VERIFICATION_INFRASTRUCTURE_V1 -->
继承 `governance/standards/FORMAL_VERIFICATION_INFRASTRUCTURE_STANDARD.md`。每个 active Problem 的首个计划必须路由出 formalization readiness：给出 expected statement、proof assistant profile 与最小可验证义务，或记录精确 obstruction。不能等自然语言 proof 宣称完成后才考虑形式化；也不能因 Lean 暂不可用而阻止 discovery/探索或把 blocker 当反例。
